// Fixed version with working tooltip, legend, and multi-vital per surgery view
...
Promise.all([
  ...
  function updateChart() {
    const selectedVitals = Array.from(vitalSelect.property("selectedOptions"), opt => opt.value);
    ...
    visible.forEach(group => {
      const [groupName, vitalName] = group.key.split(" - ");
      const groupColor = color(group.key);

      svg.selectAll(`.hover-point-${group.key}`)
        .data(group.values)
        .enter()
        .append("circle")
        ...
        .on("mouseover", function(event, d) {
          ...
          tooltip
            .style("opacity", 1)
            .html(`<strong>${vitalName.toUpperCase()}</strong><br>Value: ${d.value?.toFixed(1) ?? "N/A"}<br>Mean: ${d.mean?.toFixed(1) ?? "N/A"}<br>SD: ${d.sd?.toFixed(1) ?? "N/A"}<br>Z-score: ${zScore}`)
    ...
    const legendLabels = summary.map(d => {
      const [group, vital] = d.key.split(" - ");
      return activeGroups.size <= 1 ? vital.toUpperCase() : group;
    });

    const uniqueLabels = [...new Set(legendLabels)];
    color.domain(uniqueLabels);
